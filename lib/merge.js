const { logger, NeutralExitError, retry, getLabels } = require("./common");
...
async function merge(context, pullRequest, head) {
  ...
  const { mergeLabel, updateAndMergeLabel } = getLabels(config);
  const actionableLabels = [mergeLabel, updateAndMergeLabel];
