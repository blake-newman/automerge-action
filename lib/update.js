const { logger, NeutralExitError, getLabels } = require("./common");
...
async function update(context, dir, url, pullRequest) {
  ...
  const { mergeForks, updateMethod } = config;
  const { updateLabel, updateAndMergeLabel } = getLabels(config);
  const actionableLabels = [updateLabel, updateAndMergeLabel];
  const prLabels = pullRequest.labels.map(({ name }) => name);
  const label = actionableLabels.find(actionableLabel => prLabels.includes(actionableLabel));
  ...
  logger.info(actionableLabels, pullRequest.labels);
  if (label === undefined) {
